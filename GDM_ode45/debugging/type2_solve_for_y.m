...
phi2 = 1.0;     % Barrier at right side: In this code, set them equal!
...
%         ymin_1 = y1*(8e-1);
        ...
        coefficients = [delta,kbT,mu0,L,Nt,epi0,epir,e,a,esig,J(p),fmin_1,Inf,y1,y2,f_lower,f_upper];
        [i1,fmin_1,warning_flag,vpa_flag] = calc_i_type2_ode45_GDM(coefficients,0);
        if warning_flag~=0
            fprintf('Serious Warning: f range [f_lower, f_upper] not large enough, or y2 too big!\n');
            fprintf('fmin_initial = %f\n',fmin_1);
        end
        while vpa_flag == 1
        ...
        coefficients = [delta,kbT,mu0,L,Nt,epi0,epir,e,a,esig,J(p),fmin_2,Inf,y1,y2,f_lower,f_upper];
        [i2,fmin_2,warning_flag,vpa_flag] = calc_i_type2_ode45_GDM(coefficients,0);
        if warning_flag==1
        ...
        interp_para = [fmin_1 J1;fmin_2 J2];
        f_min_start = interp1(log10(interp_para(:,2)),(interp_para(:,1)),log10(J(p)),'linear','extrap');
%         f_min_start = 10^(y_min_start_log10);

        measure = abs((J2-J1)/J2);
        count = 0;
        while measure>tolerence
            coefficients = [delta,kbT,mu0,L,Nt,epi0,epir,e,a,esig,J(p),f_min_start,Inf,y1,y2,f_lower,f_upper];
            [i_start,f_min_start,warning_flag,vpa_flag] = calc_i_type2_ode45_GDM(coefficients,0);
            if warning_flag~=0
                fprintf('Serious Warning: f range [f_lower, f_upper] not large enough, or y2 too big!\n');
            end
            J_start = epi0*epir*mu0/(Nx*a)^3*(kbT)^2 * i_start;
            interp_para = [interp_para; f_min_start J_start];
...
%             y_min_start = 10^(y_min_start_log10);
       ...
       %% With f_end calculated, now find voltage, with equation (B11)
        f_min_end = f_min_start;
        coefficients = [delta,kbT,mu0,L,Nt,epi0,epir,e,a,esig,J(p),f_min_end,Inf,y1,y2,f_lower,f_upper];
        [f_end,y_end,f_min_end] = calc_i_type2_ode45_returnFY_GDM(coefficients,0);
